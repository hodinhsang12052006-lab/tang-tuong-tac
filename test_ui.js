...
async function runTest() {
    console.log('\n=======================================');
    console.log('🚀 BẮT ĐẦU CHẠY BÀI KIỂM THỬ E2E TRỰC QUAN (PLAYWRIGHT)');
    console.log('=======================================\n');
    ...
    const username = `test_e2e_${timestamp}`;
    const email = `test_e2e_${timestamp}@gmail.com`;

    console.log(`[Bước 2.1] Tạo tài khoản mới: ${username}...`);
    await page.fill('#register-identifier', email);
    ...
    console.log('[Bước 5] Mở một trang mới (Tab Admin)...');
    const adminPage = await adminContext.newPage();
    console.log('[Bước 5.1] Truy cập trang quản trị http://localhost:3005/admin...');
    ...
    // Thiết lập dialog auto accept
    adminPage.on('dialog', async dialog => {
        console.log(`[Hộp thoại] Chấp nhận thông báo: ${dialog.message()}`);
        await dialog.accept();
    });
    ...
    console.log('[Bước 7] Trở về màn hình Khách hàng (Tab 1)...');
    ...
    await page.click('#nav-dashboard'); // Trở lại dashboard nếu cần
    await page.waitForTimeout(1000);
    ...
    page.on('dialog', async dialog => {
        console.log(`[Hộp thoại] Chấp nhận thông báo đơn hàng: ${dialog.message()}`);
        await dialog.accept();
    });
